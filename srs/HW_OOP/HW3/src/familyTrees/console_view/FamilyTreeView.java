...
public class FamilyTreeView {
  ...
  public void showParents(String childName, int yearOfBirth) {
    ...
    if (child != null) {
      String father = Optional.ofNullable(child.getFather()).map(Human::getNAME).orElse("Неизвестно");
      String mother = Optional.ofNullable(child.getMother()).map(Human::getNAME).orElse("Неизвестно");
